...
def get_logo_filepathlist( logo_dirpath ):
	...
	logo_filepathlist = [ {keys.MY_SERVICE_NAME: x[0], keys.VIDEO_TYPE: x[2]} for x in logo_basename_tokenlist ]
	[ x.update( get_logo_filepath( x[keys.MY_SERVICE_NAME], x[keys.VIDEO_TYPE], logo_dirpath )) for x
		in logo_filepathlist ]

	return logo_filepathlist

def get_logo_filepath( my_service_name, video_type, logo_dirpath):
...
def main():
	...
	logo_filelist = get_logo_filepathlist( sys.argv[1] )
	print( logo_filelist )
